...
def via_stack(
    ...
    **kwargs: Any,
    ...
    if metal_level >= 1:
        ...
        m1_x = con.xsize + 2 * _m_enc[0]
        m1_y = con.ysize + 2 * _m_enc[1]
        m1 = c.add_ref(gf.components.rectangle(size=(m1_x, m1_y), layer=base_layer))
        m1.xmin = con.xmin - _m_enc[0]
        m1.ymin = con.ymin - _m_enc[1]
    ...
    if metal_level >= 2:
        ...
        if (via1.xmax - via1.xmin + 2 * _m_enc[0]) < (
            via_size[0] + 2 * via_enc[0]
        ) and metal_level >= 3:
            m2_x = via_size[0] + 2 * via_enc[0]

        else:
            m2_x = via1.xmax - via1.xmin + 2 * _m_enc[0]

        if (via1.ymax - via1.ymin + 2 * _m_enc[1]) < (
            via_size[1] + 2 * via_enc[1]
        ) and metal_level >= 3:
            m2_y = via_size[1] + 2 * via_enc[1]

        else:
            m2_y = via1.ymax - via1.ymin + 2 * _m_enc[1]
